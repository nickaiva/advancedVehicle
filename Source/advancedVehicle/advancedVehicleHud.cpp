...
void AadvancedVehicleHud::DrawHUD()
{
	...
	if( bHMDDeviceActive == false )
	{
		...
		if ((Vehicle != nullptr) && (Vehicle->bInCarCameraActive == false))
		{
			...
			// Chronometer
			FCanvasTextItem ChronoTextItem(FVector2D(HUDXRatio * 805.f, HUDYRatio * 555), Vehicle->ChronoDisplayString, HUDFont, FLinearColor::White);
			ChronoTextItem.Scale = ScaleVec;
			Canvas->DrawItem(ChronoTextItem);
